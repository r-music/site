---
title: "Introduction to the gm package"
description: |
...
knitr::opts_chunk$set(
  collapse = TRUE, echo = TRUE, ache = TRUE, 
...
It provides an elegant and intuitive language, with which you can easily create
music in `R`.  It generates and embeds musical scores and audio files in R Markdown documents, R Jupyter Notebooks and RStudio seamlessly.
...
let's first learn how to install and configure the `gm` package.
...
To use the `gm` package, MuseScore, an open source and free notation software, 
is required. You can install it from <https://musescore.org/>.
...
`gm` interprets MIDI note numbers based on piano keys and neighbor pitches. In the
...
We can go further with these two functions. For example, we can change `unit` 
...
![](figures/nested_tuplets_finale.png)
